...
> ✅ **HC-50 + HC-53 landed 2026-07-19.** `renderer/` (3 levels, deterministic, facts-only) and `tasks/`
> (Task-B builder + leakage audit + frozen splits). Frozen NHANES-J corpus: **4,806 patients → 14,418 cases,
> leakage audit passes with 0 forbidden-token hits and 0 label-column hits**; checksums in
> `benchmarks/task_b_manifest.json`, re-checkable with `tasks.verify()`. Three decisions worth carrying into
> the methods section:
> 1. **Difficulty changes presentation, never information.** All three levels carry the same
>    decision-relevant facts. Levels that withheld facts would confound extraction difficulty with
>    answerability and make the per-level breakdown (the whole point of HC-80) uninterpretable.
> 2. **Splits are patient-level and growth-stable.** A patient's three levels come from one structured row,
>    so per-case splitting would let a model memorizing patients score as generalizing. Hash-threshold
>    assignment (not shuffle-and-slice) means adding cycles or MIMIC never repartitions the existing
>    benchmark and never silently invalidates a published number.
> 3. **BP is floored, not rounded, for display.** NHANES BP is a mean (131.667 mmHg), which no clinician
>    charts. Rounding crosses staging thresholds — 129.6 → 130 shows a BP one stage above its own hidden
>    label — and **would have corrupted 93 of 4,806 profiles**. Flooring corrupts zero and provably cannot,
>    since every threshold is an integer. *HC-70 must score extracted SBP against the displayed value, not
>    the raw float.*
>
> ⚠️ **Two halves of the gate, only one closed.** The mechanical audit passes on all 14,418 cases. The
> **human 20-vignette facts-only read is still open** (Aria; sheet generated by `tasks/spotcheck.py`,
> stratified by decision so abstentions and contraindication-positives are over-represented). A scanner only
> catches the tokens it was told about; the human read is what catches a vignette that gives the answer away
> in wording nobody thought to ban. **HC-51 is not built**, so `hard` is currently *harder to extract from*,
> not *harder to reason about* — worth knowing before reading anything into a flat simple-vs-hard curve at
> the pilot.
