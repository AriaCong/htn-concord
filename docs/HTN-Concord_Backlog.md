...
| HC-101 | **Rebuild the difficulty ladder (K2)** | the HC-80 pilot fired K2: `hard` was inert — extraction F1 0.918 at all three levels to three decimals, and **identical per-field counts** at every level. Two mechanisms, both aimed at scored fields: `hard` now gives the **individual BP readings** (integers summing to `n ×` the displayed value, so the mean is exactly what the evaluator compares against, and no offset is zero so the mean is never on the page) and states an **earlier potassium** beside the current one (always labelled "today"; potassium reaches a label by one threshold only, so a same-side decoy is provably decision-inert). **eGFR deliberately excluded** — it enters PREVENT as two continuous spline terms, so any alternative value moves `prevent_10yr`; conservative call flagged for HC-49. Averaging convention **disclosed in the prompt**, else the drop would measure guessing (HC-100). BP's information invariant restated as *recovery*, not substring presence — strictly stronger. Corpus rebuilt: labels checksum unchanged, `simple`/`moderate` byte-identical across all 4,806 patients, audit 0 hits. **MEASURED 2026-09-23 (120/120 calls): the mechanisms did not work.** Extraction F1 **0.9182 at all three levels**, spread **0.00 pp** vs a 5 pp threshold; **K2 fired again**. The change reached the corpus and was handled perfectly — at `hard`, sbp 40/40, dbp 40/40, potassium 38/38 — so *"the manipulations were too weak"* is now **ruled out** for the strongest mechanisms the renderer allows. Presentation-only difficulty costs a frontier model nothing; (c) and (d) are weaker on the same axis and **should not be attempted**. → **HC-51 is the critical path**, not an enhancement. Freeze stays blocked. Results: `docs/HTN-Concord_HC101_Results_2026-09-23.md` (EN/CN). Provisional until the scoped `hard` re-read is signed | HC-50, HC-80 | M | 🟡 (measured; awaiting re-read) |
| HC-102 | Negative findings are scored but never rendered | found re-scoring HC-80 field by field. The renderer states diabetes **only when true**, so `diabetes=False` is never in the vignette — while the evaluator scores it and the prompt says to use null for anything not stated. The model complies, returns null, and is marked wrong: **80/95 cases**, scoring 0.158 where every other field except `on_bp_meds` is 1.000, costing **~7.6 pp of extraction F1 at every level**. Level-invariant, so it cancels out of K2 and did not block HC-101 — but the pilot's absolute extraction numbers understate the models and must not be quoted as a capability estimate. Same class as HC-100. `on_bp_meds` (0.900) is the next candidate and unexamined | HC-70, HC-50 | S | ⬜ |
