...
| ID | 工单 | 验收标准 | 依赖 | 规模 | 状态 |
|---|---|---|---|---|---|
| HC-50 | 病例渲染器（3 档难度） | `render(profile, level, seed)`；只含事实；确定性 | HC-18/HC-10 | L | ⬜ |
| HC-51 | 干扰项库 | 手写、经一次性验证；家族史 vs 病人事实的陷阱 | HC-50 | M | ⬜ |
| HC-52 | Task A 构建器 | 干净档案 → 引擎标签 | HC-33..38 | S | ⬜ |
| HC-53 | Task B 构建器 + 泄漏审计 | 渲染病例 → 隐藏标签；文本中无标签 token | HC-50、HC-51、HC-46 | M | ⬜ |
| HC-46 | 泄漏审计 lint（禁用词扫描器） | 任何面向模型的文本若含类别名/决策动词/「……的禁忌症」即判失败 | HC-29 | S | ⬜ |
