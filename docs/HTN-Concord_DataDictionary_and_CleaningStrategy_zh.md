...
| **DEMO_J** | `RIDEXPRG` | 妊娠状态 | 1 = 妊娠。**已在磁盘上但当前未被使用** —— 见清洗规则 2 |
...
3. **血压汇总 —— 用中位数（2026-07-18 统一）。** 此前本文写的是*均值*，而 Master Plan 与 MIMIC 队列算法写的都是*中位数*；该矛盾现已统一裁定为**处处用中位数**，以免主打的 Task-B 对比被「各源汇总统计量不同」所污染。在 NHANES 里这个选择几乎无代价 —— **4,783 / 4,806（99.5%）** 有全部三次读数，均值与中位数差异微乎其微 —— 而在 MIMIC 里 OMR 读数次数不规则且右偏，中位数明显更稳健。要求至少 1 组有效读数。范围核查：SBP 60–290、DBP 30–200；不合理 → `NA`（绝不截断）。
   > 🚨 **已决定但从未实现（2026-07-18 发现，工单 HC-23）。** `pipelines/nhanes/clean.py` 至今仍在计算 `df[sys_cols].mean(axis=1)` / `df[dia_cols].mean(axis=1)`。本项目的**每一份**文档 —— 本文件、Master Plan、MIMIC 队列算法、以及中英两棵 Notion 树 —— 都声称中位数已于 2026-07-18 统一，但没有任何代码实现它。**因此已交付的 `nhanes_profiles_J.csv` 是一份基于均值的产物**，而任何地方引用的分期/标签数字（Stage 1+2 = 42.8%、4,806 行的标签分布）描述的都是*均值*管线。
   > 改成中位数会重新导出基准底料并移动这些数字，所以这是一次**基准冻结级决策，而不是清理** —— 要有意识地去做，然后一次性完成重跑与全部下游数字的重新引用。在 HC-23 落地之前，请把本文里的「中位数」读作*意图*中的规则，而非已实现的规则。
...
4. **分期**（引擎输入，在清洗之后计算）：Elevated 120–129/<80；**Stage 1 130–139 或 80–89**；**Stage 2 ≥140 或 ≥90**。使用**中位数** SBP/DBP（依上文 §3 —— 本行此前写的是*均值*，与同一文档的 §3 自相矛盾；2026-07-18 更正）。
...
| 派生字段 | 定义 | 输入 |
|---|---|---|
| `sbp`、`dbp` | 有效读数的**中位数**（NHANES）/ 解析后 OMR 的中位数（MIMIC）/ —（eICU 急性） | 各源血压 |
...
(e) **中位数血压统一（HC-23）→ 已决定但未实现。** 见 §1 规则 3 的警示框。
